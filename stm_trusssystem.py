...
class TrussSystem:
    ...
    def solveTruss(self, update=False, A_min=0, d_max=None):
        ...
        self.EdgeForces, self.nodal_disp, restrainedForces, \
            self.ShearForces, self.BendingMoments = \
            self.solvermethod.solveSystem(
                self.NodeList,
                self.EdgeList,
                self.numDofs,
                self.ForceList,
        ...
        if len(self.nodal_disp[0]) == 1:
            self.nodal_disp = np.reshape(
                self.nodal_disp,
                (len(self.nodal_disp) // 3, 3)
            )
    
        # ========================================================
        # UPDATE FORCES AND AREAS
        # ========================================================
    
        for i in range(len(self.EdgeList)):
    
            self.EdgeList[i].UpdateForce(
                self.EdgeForces[i]
            )
        ...
        if update:
            ...
            for i, e in enumerate(self.EdgeList):
                ...
                # Positive force = tension = tie
                if e.force > 0:
    
                    self.total_steel_area += e.area
            ...
            print('----------------------------------------')
            print(
                'TOTAL STEEL AREA = {:.2f} mm²'.format(
                    self.total_steel_area
                )
            )
            print('========================================\n')
    ...
    def plotStressField(self, fig_size=(12, 6), savefig: str = None, hydrostaticNodes=True, t=1, tol = 20.0, polygon_in = None, polygon_out = None, print_disc_points = False, check_disc_points= False):
